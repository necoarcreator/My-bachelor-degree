...
{
	long long int N = stod(argv[1]), L = N * M_PI / 24;
	//2 * M_PI;
	...
	//double thetta0 = stod(argv[12]) * K, x0 = stod(argv[12]);

	//double ampGauss = A * stod(argv[9]), meanGauss = stod(argv[10]), sygGauss = stod(argv[11]);
	...
	if (isSeries) {
		...
		signal = vector<complex<double>>{ firstStartBSOLSeriesSinNoize(x, K, a, b, A, alpha, thetta0, x0, coeff1, coeff2) };
	}
	//else {
	//	//riccati
