...
vector<complex<double>> firstStartBSOLSerieCollision(vector<double> x, double k1, double a1, double b1, double A1, double alpha1, double thetta01, double x01, double k2, double a2, double b2, double A2, double alpha2, double thetta02, double x02)
{	//bright solitary wave if it is derived from equations for series
	size_t N = x.size();
	vector<complex<double>> signal(N);
	uniform_real_distribution<double> unif(-1., 1.);
	...
	for (size_t n = 0; n < N; n++)
	{
		signal[n] = A1 * (cos(k1 * x[n] - thetta01) + i * sin(k1 * x[n] - thetta01)) / (a1 * exp(alpha1 * (x[n] - x01)) + b1 * exp(-alpha1 * (x[n] - x01))) + A2 * (cos(k2 * x[n] - thetta02) + i * sin(k2 * x[n]- thetta02)) / (a2 * exp(alpha2 * (x[n] - x02)) + b2 * exp(-alpha2 * (x[n] - x02)));
	}
	return signal;
}
...
{
	long long int N = stod(argv[1]), L = N * M_PI / 18;
	//2 * M_PI;
	...
	double thetta0 = stod(argv[9]) * K, x0 = stod(argv[9]);

	double A2 = stod(argv[10]), alpha2 = stod(argv[11]), x02 = stod(argv[12]), thetta02  = stod(argv[13]);

	unsigned fo  = 100;
	
	//double coeff1 = stod(argv[9]);
	//cout << "coeff1  = " << coeff1  << endl;
	//double coeff2  = stod(argv[10]);
	...
	if (isSeries) {
		...
		if (tau > pow(h, 2) / 20 / abs(chi)) tau = pow(h, 2) / 20 / abs(chi);
	...
	if (isSeries) {
		...
		signal = vector<complex<double>>{ firstStartBSOLSerieCollision(x, K, a, b, A, alpha, thetta0, x0, K, a, b, A2, alpha2, thetta02, x02) };
	}
	//else {
	//	//riccati
