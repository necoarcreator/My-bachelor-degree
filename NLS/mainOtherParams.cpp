...
vector<complex<double>> firstStartBSOLSerieCollision(vector<double> x, double k1, double k2, double a, double b, double A1, double A2, double alpha, double thetta0, double x0,double thetta1, double x1)
{	//bright solitary wave if it is derived from equations for series
	size_t N = x.size();
	vector<complex<double>> signal(N);
	...
	for (size_t n = 0; n < N; n++)
	{
		signal[n] = A1 * exp(i * (k1 * x[n] - thetta0)) / (a * exp(alpha * (x[n] - x0)) + b * exp(-alpha * (x[n] - x0))) + A2 * exp(i * (k2 * x[n] - thetta1)) / (a * exp(alpha * (x[n] - x1)) + b * exp(-alpha * (x[n] - x1)));
	}
	return signal;
}
...
void linearFM(vector<complex<double>>& signal, vector<double> omega, vector<double> x, double tau, long long int L, double chi)
{
	...
	{
		(signal)[k] *= exp(i * tau * (-1.0 * pow(omega[k], 2) + pow(omega[k], 3) + chi * pow(omega[k], 4)));
	}
	//inversed for linear
...
{
	long long int N = 256, L = N * M_PI / 48;
	//2 * M_PI;
	double T = 5.0;
	...
	double thetta0 = M_PI / 2, x0 = -7.5;
	
	double thetta1 = 0.0, x1 =  7.5;

	cout << "A1 = " << A1 << endl;
	
	cout << "A2 = " << A2 << endl;

	tau = pow(h, 4) / (2 * abs(chi));
	...
	if (isSeries) {
		...
		omega = -(256 * alpha * alpha * pow(chi, 4) + 256 * alpha * alpha * pow(chi, 3) + 192 * alpha * alpha * chi * chi + 64 * chi * chi - 16 * chi - 1) / (256 * pow(chi, 3));

		cout << "omega = " << omega << endl;

		eps = 96 * pow(chi, 3) / (alpha * alpha * (1600 * pow(chi, 4) + 320 * pow(chi, 3) + 256 * pow(chi, 2) + 24 * chi + 9));
		
		cout << "eps = " << eps << endl; 

		v = - 1 / (16 * chi * chi) - 1 /(2 * chi) + 1; //series
		
		cout << "v = " << v << endl;

		if (tau > pow(h, 2) / 2) tau = pow(h, 2) / 2;
	...
	if (isSeries) {
		...
		outputFile1.open("test1re.csv");
		outputFile2.open("test1im.csv");
		signal = vector<complex<double>>{ firstStartBSOLSerieCollision(x, K1, K2, a, b, A1, A2, alpha, thetta0, x0, thetta1, x1) };
	} 
