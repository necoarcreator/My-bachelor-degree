...
void filtrationSmooth(vector<complex<double>>& q, vector<double> omega, vector<double> x, double h, double alpha, double kCutDole, double L, size_t step, int num_threads) {

	size_t N = q.size();

	// âû÷èñëÿåì ïîëóøèðèíó îêíà â èíäåêñàõ
	...
	// Äåëàåì ìÿãêîå îòñå÷åíèå âûñîêèõ ÷àñòîò
	...
	// Ñ÷èòàåì, ñêîëüêî "ýíåðãèè" (I1, I2) ïîòåðÿëîñü
...
void filtration(vector<complex<double>>& q, double h, double factor, double L, size_t step) {
	...
	// Ôèëüòðóåì (äåëèì íà factor) âñå ýëåìåíòû ÂÍÅ [i_left, i_right]
	// Ó÷èòûâàÿ, ÷òî èíäåêñ ìîæåò "ïåðåâàëèâàòüñÿ" ÷åðåç ãðàíèöó ìàññèâà.

	if (i_right < i_left) {
		// Ñ÷èòàåì, ÷òî îêíî "îáåðíóëîñü" ÷åðåç êîíåö ìàññèâà.
		// Òîãäà ôèëüòðóåì U[i_right..i_left].
		for (int i = i_right; i < i_left; i++) {
			q[i] *= exp(-factor * pow(abs(q[i] - maxAmp), 2));
		}
	}
	else {
		// Ñíà÷àëà ôèëüòðóåì "ëåâóþ" ÷àñòü [0..i_left], 
		// ïîòîì "ïðàâóþ" ÷àñòü [i_right..N-1].
		// (Òóò ïîäðàçóìåâàåòñÿ, ÷òî "îêíî" - ýòî (i_left..i_right),
		//  è ìû, íàîáîðîò, çà åãî ïðåäåëàìè îñëàáëÿåì ðåøåíèå.)
...
{	//bright solitary wave if it is derived from equations for series
	...
	for (size_t n = 0; n < N; n++)
	{
		signal[n] = A * exp(i * (k * x[n] - thetta0)) / (a * exp(alpha * (x[n] - x0)) + b * exp(-alpha * (x[n] - x0))) + ampl * exp(-pow(x[n] - mn, 2.0) / pow(syg, 2.0));
	}
	return signal;
}
...
void linearFM(vector<complex<double>>& signal, vector<double> omega, vector<double> x, double tau, long long int L, double chi, int num_threads)
{
	FT fftLin(omega, num_threads);
...
void nonlinearFM(vector<complex<double>> U, vector<complex<double>>& V, vector<double> omega, double tau, long long int L, double eps, int num_threads)
{
...
{
	long long int N = 8, L = N * M_PI / 16;
	//2 * M_PI;
	double T = 0.5, start, end;
	...
	double thetta0 = 0., x0 = 0.;

	cout << "A = " << A << endl;

	tau = pow(h, 4) / 2;

	double ampGauss = A / 2, meanGauss = 2.5, sygGauss = 2.0;
	...
	if (isSeries) {
		...
		outputFile1.open("a2re.csv");
		outputFile2.open("a2im.csv");
		signal = vector<complex<double>>{ firstStartBSOLSeries(x, K, a, b, A, alpha, thetta0, x0) };
	}
	//else {
	//	//riccati
	...
	//	omega = 3 * a4 * pow(k, 4) + 12 * a4 * b * pow(k, 2) + a2 * pow(k, 2) - 16 * a4 * pow(b, 2) + a1 * k + 2 * a2 * b;

	//	b1 = 2 * (6 * a4 * pow(k, 2) - 20 * a4 * b + a2) / pow(A, 2);

	//	b2 = 24 * a4 / pow(A, 4);

	//	outputFile.open("log/bsolEnvSecTest25d.csv");

	//	signal = new vector<complex<double>>{ firstStartDSOLRiccati(x, k, b, A, thetta0, x0) };
	...
	//çàïèñü äëÿ 3-ä ãðàôèêà êâàäðàòà ìîäóëÿ ñèãíàëà
	...
	if (outputFile1.is_open() and outputFile2.is_open())
	{
		...
		try {
			for (size_t p = 0; p < M; p++)
			{
				...
				//ìîäóëè ñ÷èòàþòñÿ ïî ñòàðûì êîìïîíåíòàì
